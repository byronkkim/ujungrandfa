...
- 🥚 **치트키 이스터에그**: 상단바 오른쪽 끝의 흐릿한 `▼`를 누르면 입력칸+제출 버튼이 나오고, `이재온`을 넣으면 하트 상한이 2배(탱크 5→10, 검사 3→6). **한 판에 한 번만** 쓸 수 있고 발동하면 화살표가 사라진다. 틀리면 "땡!"만 뜨고 재시도 가능, 새 판(다시 하기)이면 초기화. 구현: [`src/app/game/page.tsx`](src/app/game/page.tsx)의 `submitCheat()` + `tankGame.ts`의 `setCheat()`
- 😈 **고수 모드**(시작 화면 체크박스): 탱크 없이 **바로 전사로 시작**, **한 대만 맞으면 게임 오버**, **필살기 없음**, 적 체력 상한 3→**9**, 보스 체력 **5배**에 이동 1.4배·연사 강화. 켜면 시작 단계가 **7단계**로 맞춰진다
